#!/usr/bin/env python3
"""Generate Hunyuan-DiT samples from an explicit prompt_id-ordered manifest."""
...
def main():
    ...
    print(
        f"[hunyuan-manifest] manifest={args.prompt_manifest} prompts={len(rows)} "
    ...
    with open(meta_path, "a", encoding="utf-8") as meta_file:
        for row in tqdm(shard_rows, desc="Hunyuan manifest prompts"):
            pid = int(row["prompt_id"])
            prompt = row["prompt"]
            missing_sids = [
                sid
