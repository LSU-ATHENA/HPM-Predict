#!/usr/bin/env python3
"""Generate DreamShaper samples from an explicit prompt_id-ordered manifest."""
...
def main():
    ...
    try:
        ...
        from datagen.model_registry import DREAMSHAPER_CONFIG
    except ImportError:
        from gen_dataset.datagen.dreamshaper import DreamShaperGenerator
        from gen_dataset.datagen.model_registry import DREAMSHAPER_CONFIG
    import torch
    ...
    print(
        f"[dreamshaper-manifest] manifest={args.prompt_manifest} prompts={len(rows)} "
    ...
    print(
        "[dreamshaper-manifest] protocol="
        f"steps={DREAMSHAPER_CONFIG['num_inference_steps']} "
        f"cfg={DREAMSHAPER_CONFIG['guidance_scale']} "
        f"scheduler={DREAMSHAPER_CONFIG.get('scheduler') or 'pipeline default'}"
    )
    if not planned:
    ...
    protocol_meta = {
        ...
        "num_inference_steps": int(DREAMSHAPER_CONFIG["num_inference_steps"]),
        "guidance_scale": float(DREAMSHAPER_CONFIG["guidance_scale"]),
        "scheduler": DREAMSHAPER_CONFIG.get("scheduler"),
        "protocol_note": DREAMSHAPER_CONFIG.get("protocol_note"),
    }

    with open(meta_path, "a", encoding="utf-8") as meta_file:
        for row in tqdm(shard_rows, desc="DreamShaper manifest prompts"):
            pid = int(row["prompt_id"])
            prompt = row["prompt"]
            missing_sids = [
                sid
