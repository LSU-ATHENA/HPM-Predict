#!/usr/bin/env python3
"""Recover SDXL samples from an explicit prompt/sample worklist."""
...
def main():
    ...
    print(
        f"[sdxl-sample-worklist] worklist={args.worklist} samples={len(rows)} "
    ...
    meta_path = data_dir / f"metadata_recovery_{task_id}.jsonl"
    written = 0
    ...
    with open(meta_path, "a", encoding="utf-8") as meta_file:
        for row in tqdm(shard_rows, desc="SDXL sample worklist"):
    ...
    print(
        f"Done. metadata_written={written} recovered_metadata={recovered_metadata} "
        f"regenerated_images={regenerated_images} regenerated_noise={regenerated_noise} "
