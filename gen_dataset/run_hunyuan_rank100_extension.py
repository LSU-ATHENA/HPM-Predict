#!/usr/bin/env python3
"""Extend selected Hunyuan-DiT prompts from sample_idx 20..99 safely."""
...
def main():
    ...
    print(
        f"[hunyuan-rank100] data_dir={data_dir} selected_rows={len(manifest_rows)} "
        f"range=[{start_idx}, {end_idx}) task_id={task_id} "
        f"cross_attn={extract_cross_attn}"
    )
    ...
    print(f"[hunyuan-rank100] planned_new_samples={planned} skipped_existing_metadata={skipped_existing}")
    if args.dry_run:
        return
    ...
    generator = HunyuanDiTGenerator(
        ...
        task_id=None,
        extract_cross_attn=extract_cross_attn,
    ...
    try:
        with open(output_metadata, "a") as meta_file:
            for row in tqdm(rows, desc="Hunyuan rank100 prompts"):
                pid = int(row["prompt_id"])
                prompt = row["prompt"]
                ...
                for sid, seed in zip(new_sids, seeds):
                    ...
                    if not args.recover_existing_outputs:
                        assert_no_existing_outputs(data_dir, name, extract_cross_attn)
                    ...
                    if extractor is not None:
                        if need_forward:
                            ...
                            ca_results = torch.load(ca_path, map_location="cpu", weights_only=False)
