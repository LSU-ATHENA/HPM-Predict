#!/usr/bin/env python3
"""Generate SDXL samples from an explicit prompt_id-ordered manifest."""
...
def load_existing_metadata(data_dir: Path, task_id):
    seen_pairs = set()
    seeds_by_prompt = defaultdict(set)
    paths = [data_dir / "metadata.jsonl"]
    if task_id is not None:
        paths.append(data_dir / f"metadata_{task_id}.jsonl")
    for path in paths:
        if not path.exists():
            continue
...
def main():
    ...
    rows = load_manifest(Path(args.prompt_manifest))
    ...
    existing_pairs, seeds_by_prompt = load_existing_metadata(data_dir, task_id)
    planned = []
    target_sids = set(range(args.images_per_prompt))
    ...
    print(
        f"[sdxl-manifest] manifest={args.prompt_manifest} prompts={len(rows)} "
    ...
    meta_path = data_dir / (f"metadata_{task_id}.jsonl" if task_id is not None else "metadata_generated.jsonl")

    with open(meta_path, "a", encoding="utf-8") as meta_file:
        for row in tqdm(shard_rows, desc="SDXL manifest prompts"):
            pid = int(row["prompt_id"])
            prompt = row["prompt"]
            missing_sids = [sid for sid in range(args.images_per_prompt) if (pid, sid) not in existing_pairs]
            if not missing_sids:
                continue
